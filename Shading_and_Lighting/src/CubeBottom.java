...
public class CubeBottom
{
	...
	public CubeBottom(float _x, float _y, float _z, float _size, int _stacks, int _slices)
	{
	...
	private void fillMesh()
	{
		...
		for (i = 0, row_layer = center.z - (size/2); i < stacks; i++, row_layer += stack_step)
		{
			for (j = 0, col_layer = center.x - (size/2); j < slices; j++, col_layer += slice_step)
			{
				mesh.v[i][j].x = (float) col_layer;
				mesh.v[i][j].y = (float) center.y + (size/2);
				mesh.v[i][j].z = (float) row_layer;
				
				mesh.n[i][j].x = (float) 0;
				mesh.n[i][j].y = (float) 1;
